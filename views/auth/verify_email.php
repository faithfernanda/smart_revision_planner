...
<head>
    ...
    <title><?= __('verify_email_title') ?> - <?= site_name() ?></title>
...
<body class="bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-800 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white/10 backdrop-blur-xl border border-white/20 p-8 md:p-10 rounded-3xl shadow-2xl w-full max-w-md relative z-10 animate__animated animate__fadeInUp">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-extrabold text-white mb-2 flex items-center justify-center gap-3"><?= __('verify_email_title') ?> <i data-lucide="mail" class="w-8 h-8 text-indigo-400"></i></h1>
            <p class="text-indigo-200 text-sm"><?= __('verify_email_subtitle') ?></p>
        </div>
        ...
        <?php if(isset($_GET['resend'])): ?>
            <div class="bg-blue-500/20 border border-blue-500/50 text-blue-100 p-4 rounded-xl mb-6 text-sm flex items-center gap-2">
                <i data-lucide="info" class="w-5 h-5 flex-shrink-0"></i>
                Veuillez vérifier votre compte avant de vous connecter. Un nouveau code a été envoyé.
            </div>
        <?php endif; ?>

        <form action="<?= url('/verify-email') ?>" method="POST" class="space-y-6">
            <?= csrf_field() ?>
            <input type="hidden" name="user_id" value="<?= $id ?>">
            <div>
                <label class="block text-indigo-200 text-sm font-bold mb-2 ml-1">Code de vérification</label>
        ...
        <div class="mt-8 text-center border-t border-white/10 pt-6">
            <p class="text-sm text-indigo-200">
                ...
                <a href="<?= url('/resend-verification?id=' . $id) ?>" class="text-white font-bold hover:text-pink-300">Renvoyer</a>
            </p>
        </div>
    </div>
